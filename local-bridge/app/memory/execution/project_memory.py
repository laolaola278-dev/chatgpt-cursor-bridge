...
class ExecutionMemory:
    DOCUMENTS = {
        ...
        "failures": "failure-patterns.md",
        "learning": "engineering-lessons.md",
    }

    def __init__(self, settings: Settings) -> None:
        self.settings = settings
        self.root = settings.memory_root / "execution"

    def preview(self, project: str, category: str, content: str) -> str:
        cleaned = sanitize_content(content, self.settings)
        return f"[execution memory proposal/{category}] {self._path(project, category).name}\n\n{cleaned[:1200]}"

    def _path(self, project: str, category: str, create_dir: bool = False) -> Path:
        validate_project_name(project)
        if category not in self.DOCUMENTS:
            raise ValidationFailed("Unknown execution memory category")
